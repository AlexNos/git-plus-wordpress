 ...
 **/
if (is_admin()) {
	$woo_plugin_updater_software_addon = new WooThemes_Plugin_Updater( __FILE__ );
	$woo_plugin_updater_software_addon->api_key = 'f8ed5e6fa6d52207159755f53c88540f';
	$woo_plugin_updater_software_addon->init();
}
...
CREATE TABLE ". $wpdb->prefix . "woocommerce_software_activations (
  ...
  PRIMARY KEY  (activation_id)
					...
					if ( $item['id'] > 0 ) {
						...
						if ( $meta['_is_software'][0] == 'yes' ) {
			                ...
			                $data = array(
								...
								'prefix'				=> empty( $meta['_software_license_key_prefix'][0] ) ? $this->generate_licence_key() : $meta['_software_license_key_prefix'][0],
		...
		function validate_add_cart_item( $passed, $product_id, $qty ) {
			...
			if ( ! empty( $_POST['upgrade_software'] ) && $is_software == 'yes' ) {
