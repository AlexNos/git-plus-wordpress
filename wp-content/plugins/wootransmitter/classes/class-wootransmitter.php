...
class WooThemes_Transmitter {
	...
	public function __construct ( $file ) {
		require_once( 'class-wootransmitter-updater.php' );

		$this->updater = new WooTransmitter_Updater( $file );
	...
	public function add_toolbar_menu () {
		...
		// Main Menu Item
		$wp_admin_bar->add_menu( array( 'parent' => 'top-secondary', 'id' => $this->token, 'title' => $menu_title, 'href' => '#' ) );
	...
	private function request ( $endpoint = '/messages', $params = array() ) {
		...
		if( is_wp_error( $response ) ) {
		  $data = new StdClass();
		  $data->error = __( 'WooTransmitter Request Error', 'wootransmitter' );
		} else {
			$data = $response['body'];
		}
	...
	public function enqueue_styles () {
		wp_register_style( $this->token, $this->assets_url . 'css/style.css', 'screen', '1.0.1' );
